...
| `today.ts` | The `users/{uid}/today/{date}` subcollection (#98): gathers the ladder's inputs, calls it, fills the template from `content.ts`, chooses the day's banner rail with the card (#102), records the shortcuts row's mode / period / nutrition-setup facts (#100), caches all of it, pages them out for `GET /me/export` without the cache bookkeeping (#58), deletes them all. Also **the seam the cycle maths is read through** — `cycleToday` for the card and the shortcuts row, `cycleAnalysisFor` for the calendar (#205) | The only module that touches `today/`. The card's rung and template id come from the subject, never from a phraser. The cache write reads the account in its transaction (#286). Both cycle readers share one event window, one `CycleDay` mapping and one call to `analyzeCycles` |
...
| `GET /me/nutrition/plan` | Bearer | `{ plan }` — the day's targets for a finished setup (S2 #222, served first here for S3 #223): `planDailyTargets`' answer, `{ kind: "targets", targets }` or `{ kind: "refused", refusal: { reason, lowestSupportedWeightKg } }` (the BMI floor or the per-plan cap, with the value to offer). `400 VALIDATION` while setup is incomplete or the body metrics are (PRD line 677: nothing from partial data); `503 SERVICE_UNAVAILABLE` while the `NUTRITION_*` constants are unconfigured |
...
mode            'cycle' | 'planning' | 'pregnancy' | 'postpartum' | 'loss'   // D1's (#100)
periodOngoing   boolean | null  // her logged period still running today (#100)
nutritionSetUp  boolean | null  // Nutrition coach setup finished (#100)
...
**The shortcuts row's facts ride on the same document (#100, D5).** `mode` is the one D1
was handed, `periodOngoing` is `cycle.ts`'s — the menstrual phase's boundary (#197) without
the prediction gate, because it labels a logging shortcut rather than telling her a phase,
and a woman in her first three cycles is still logging her period — and `nutritionSetUp` is
`completedSetup`'s answer, the one the rail's focus areas are gated on. All three come from
inputs `dataChangedAt` already watches, so a flow entry or a finished setup regenerates the
day exactly as it regenerates the card. A day stored before #100 reads `mode: 'cycle'`
(every card before D10 was) and `null` for the other two rather than being rebuilt; `null`
and not `false`, because a fabricated "not on her period" would otherwise be in her export.
The device chooses `Log feed` / `Log period` / `Log` from these and never derives a period
from events itself.
