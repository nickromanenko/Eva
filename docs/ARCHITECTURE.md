...
| `users.ts` | The `users/{uid}` document: read, create, update | The only module that touches Firestore |
...
Layering: `index.ts` → (`auth`, `identity-toolkit`, `users`) → (`firebase`, `config`).
...
`UNAUTHORIZED`); `message` is human-facing and may be shown in the app. Changing a
code is a breaking change for the iOS client.
