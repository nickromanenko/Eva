...
describe('a cycle outside 21-45 days is excluded AND still returned, flagged', () => {
  ...
  test('21 and 45 are inside the range; 20 and 46 are the first ones outside it', () => {
    const countedAt = (length: number) => analyze(periods([length], 5)).cycles[0]?.counted ?? null
  ...
  test('an excluded cycle that drops her under the gate withdraws the prediction, with the reason in the output', () => {
    ...
    expect(mislogged.cycles.map((cycle) => cycle.excluded)).toEqual([null, null, 'unusual-length'])
  })
})
...
describe("irregularity uses the FIGO band for the user's age", () => {
  ...
  test('the band changes at 25→26 and back at 41→42', () => {
    expect([25, 26, 41, 42].map((age) => irregularAt(age, 8))).toEqual([false, true, true, false])
  })
...
describe('the variation reads every interval in the window, not only the counted ones', () => {
  ...
  test('alternating 28 and 20 days is irregular at the 7-day band', () => {
    ...
    expect(analyze(periods([28, 20, 28, 20, 28, 20], 5), profileAged(20)).irregularity).toBe('none')
  })
...
describe('one interval Eva could not count is not the same fact as cycles that vary (#190)', () => {
  ...
  test('and the card no longer tells her that her cycle lengths vary', () => {
    ...
    expect(subjectFor(periods([28, 28, 28, 28, 28, 38], 5)).templateId).toBe(TEMPLATE.irregular)
  })
  ...
  test('two intervals out of range is a pattern, not a log Eva could not read', () => {
    const twice = periods([REGULAR, MERGED, REGULAR, REGULAR, MERGED, REGULAR, REGULAR], 5)
...
describe('an age under the account floor refuses rather than banding', () => {
  ...
  test('eighteen today is in; the day before her birthday is out', () => {
    ...
    expect(() => bandForAge(eighteenToday, RULES, shift(TODAY, -1))).toThrow(ImpossibleAgeError)
  })
...
describe('no number in the maths is written in the maths', () => {
  ...
  test('moving the countable range moves which cycles count', () => {
    ...
    expect(analyzeCycles({ days: logged, today: TODAY, profile: null }, wider).countedCycles).toBe(
      3,
    )
  })
  ...
  test('moving the gate moves when a prediction appears', () => {
    ...
    expect(analyzeCycles({ days: logged, today: TODAY, profile: null }, lower).prediction).not.toBe(
      null,
    )
  })
  ...
  test('moving the luteal phase moves ovulation and the whole window with it', () => {
    ...
    const moved = analyzeCycles({ days: logged, today: TODAY, profile: null }, twelve).prediction!
  ...
  test('moving the band widths moves the narrow/wide boundary', () => {
    ...
    expect(
      analyzeCycles({ days: logged, today: TODAY, profile: null }, earlier).prediction?.confidence,
    ).toBe('narrow')
  })
})
...
describe('a set of constants that would produce a plausible wrong answer is refused', () => {
  ...
  test('a narrow band reachable before the gate, which would make the wide band dead', () => {
    refuse({ narrowBandMinCycles: 2 }, 'narrowBandMinCycles', 'at least minCyclesForEstimate')
  })
...
describe('a date that is not one is refused rather than read as no data', () => {
  test('today', () => {
    expect(() => analyzeCycles({ days: [], today: '15/06/2026', profile: null }, RULES)).toThrow(
      InvalidCycleDateError,
    )
  })
  ...
  test('a day that does not exist is refused rather than rolled forward', () => {
    ...
    expect(() => analyzeCycles({ days: [], today: '2026-04-31', profile: null }, RULES)).toThrow(
      InvalidCycleDateError,
    )
...
describe('the estimate D1 already consumes', () => {
  ...
  test('fed into D1, six regular cycles select the phase card on a follicular day only', () => {
    ...
    const onCycleDay = (cycleDay: number, flowDays: number): DashboardInput => ({
      ...
      today: TODAY,
      now: `${TODAY}T09:00:00Z`,
      cycle: toCycleEstimate(analyze(periods([28, 28, 28, 28, 28, 28], cycleDay - 1, flowDays))),
      signals: [],
      redFlag: null,
      upcomingAppointments: [],
      profileComplete: true,
...
describe('the module reaches nothing', () => {
  test('it imports in a process with no environment and no credentials', async () => {
    const { exitCode, stderr } = await importInABareProcess(`${import.meta.dir}/../src/cycle.ts`)
    expect(stderr).toBe('')
