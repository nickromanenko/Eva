...
describe("where credential calls are sent", () => {
    test(
        "the auth emulator host becomes the Identity Toolkit origin, and Google is the default",
        async () => {
            ...
            const read = async (env: Record<string, string>) => {
                const proc = Bun.spawn(
                    ["bun", "-e", "import('./src/config.ts').then(m => console.log(m.config.identityToolkitBaseUrl))"],
                    {
                        cwd: `${import.meta.dir}/..`,
                        env: { PATH: process.env.PATH ?? "", ...BASE_ENV, ...env },
                        stdout: "pipe",
