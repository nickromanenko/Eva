...
const api = (path: string, init?: RequestInit & { token?: string | null; base?: string }) =>
...
/**
 * The port range for the UTC+14 API: not the suite server's window, not answered by
 * anything, and not one this file has spawned on before (#193). `bootApi`'s draw excludes
 * the window by value (because `EVA_API_PORT` is a developer's to set) and still asks first,
 * because the window is not where every Eva API is: `EVA_API_URL` can point the suite
 * somewhere else entirely, `scripts/ci-mobile.sh` runs its own server on 3203, and a server
 * an interrupted run left behind answers `Eva API` just the same.
 */
const PORT_RANGE: [number, number] = [3100, 3299]
...
describe('events: auth and basic CRUD', () => {
  test('every event route requires a bearer token', async () => {
    ...
    expect((await api('/me/events', { method: 'POST', token: null, body: '{}' })).status).toBe(401)
    expect((await api('/me/events/x', { method: 'PATCH', token: null, body: '{}' })).status).toBe(
      401,
    )
    expect((await api('/me/events/x', { method: 'DELETE', token: null })).status).toBe(401)
    expect(
      (await api(`/me/body-signals/${today}`, { method: 'PUT', token: null, body: '{}' })).status,
    ).toBe(401)
  })
...
describe('events: timezone', () => {
  ...
  test('two devices a day apart both keep their own date', async () => {
    ...
    const eastEvent = (
      await json<EventResponse>(await post(sport(east, { timeZone: 'Pacific/Kiritimati' })))
    ).event
    const westEvent = (
  ...
  test('a server running in UTC+14 stores the same dates as this one', async () => {
    const { base, child } = await bootApi({
      env: { TZ: 'Pacific/Kiritimati' },
      range: PORT_RANGE,
      exclude: { from: suitePortsFrom, to: suitePortsTo },
      label: 'events.test.ts UTC+14',
      tries: 40,
    })
    try {
      // Backdated: same stored date and the same 12:00 default on both servers.
      const date = shiftDays(todayIn('UTC'), -40)
      const shifted = (await json<EventResponse>(await post(sport(date), base))).event
      ...
      // `bootApi`'s draw makes adoption unlikely; this makes it visible if it happens anyway.
      child.kill()
      await child.exited
      if (await answers(base)) {
        throw new Error(
          `${base} still answers with the UTC+14 API stopped: the dates ` +
...
describe('events: date policy', () => {
  test('future dates are refused for observational types', async () => {
    ...
    expect((await json<ErrorResponse>(bodySignals)).error.code).toBe('FUTURE_DATE_NOT_ALLOWED')
  })
...
describe('events: positive test', () => {
  ...
  test('it needs no flow on the day, and shares a day with a cycle entry', async () => {
    ...
    expect((await post({ type: 'positiveTest', localDate: dry, timeZone: 'UTC' })).status).toBe(201)
    ...
    expect(flow.status).toBe(201)
    expect((await post({ type: 'positiveTest', localDate: shared, timeZone: 'UTC' })).status).toBe(
      201,
    )
...
describe('events: body signals', () => {
  ...
  test('ratings are 1–5 whole numbers and symptoms are shaped', async () => {
    ...
    expect((await put({ symptoms: [{ code: 'cramps' }, { code: 'cramps' }] })).status).toBe(400)
