...
const AUDIT: Record<TemplateId, Audited> = {
  [TEMPLATE.coldStart]: {
    ...
    copy: {
      ...
      line2:
        'Log your period or today’s body signals so Eva can begin recognizing patterns that are specific to you.',
  ...
  [TEMPLATE.stillLearning]: {
    ...
    copy: {
      ...
      line2:
        'Log more periods to help estimate your cycle phases more reliably. Until then, no phase is shown.',
    ...
    claims: {
      ...
      line2: {
        ...
        holds: (c) => c.input.cycle.countedCycles < 3 && !c.input.cycle.enoughCyclesForEstimates,
      },
      actions: null,
    },
  },
  ...
  [TEMPLATE.irregular]: {
    ...
    copy: {
      ...
      line2:
        'Your recent cycle lengths vary significantly, so Eva will not show a confident prediction.',
  ...
  [TEMPLATE.moodPattern]: {
    ...
    copy: {
      ...
      line2:
        'Sleep has also been below your usual level during the same period. Eva can see the pattern but not its cause.',
    ...
    claims: {
      ...
      title: {
        ...
        holds: (c) => {
          ...
          return (
            doseOf(c).lowSignalDays === 3 && days !== null && days.every((e) => isLow(e.mood, c))
          )
        },
      },
      line2: {
  ...
  [TEMPLATE.educational]: {
    ...
    copy: {
      ...
      line2:
        'Educational content, not personalized insight — no new body signals in the last 24 hours.',
...
describe('the copy audit walks every subject the ladder can select', () => {
  ...
  test('and the space it walks is wide enough to have found them', () => {
    ...
    expect(speakablePhasesAt(TEMPLATE.educational)).toEqual(['luteal', 'menstrual', 'ovulation'])
    ...
    expect(CLOCK_CASES.map((c) => WALK.find((s) => s.case === c)!.subject.templateId)).toEqual([
      TEMPLATE.signalOverridesPhase,
      TEMPLATE.signalsToday,
      TEMPLATE.signalsToday,
    ])
...
describe('the signal phrase names only what the entry contains', () => {
  ...
  test('a low rating names that rating, in energy-mood-sleep order', () => {
    ...
    expect(phraseForEntry(entry({ energy: 1, mood: 1, sleep: 1 }), VOCAB, labelFor)).toBe(
      'low energy',
    )
  })
  ...
  test('a symptom names its label, lowercased for the sentence', () => {
    expect(
      phraseForEntry(
        entry({ symptoms: [{ code: 'headache', severity: 'severe' }] }),
        VOCAB,
        labelFor,
      ),
    ).toBe('headache')
  ...
  test('a symptom whose code has no label is skipped, never rendered as a code', () => {
    ...
    expect(
      phraseForEntry(
        entry({ symptoms: [{ code: 'not-a-real-code', severity: 'severe' }] }),
        VOCAB,
        labelFor,
      ),
    ).toBe('body signals')
