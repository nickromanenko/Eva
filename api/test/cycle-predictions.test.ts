...
describe('GET /me/cycle/predictions: the range', () => {
  ...
  test('is validated and capped exactly as GET /me/events is', async () => {
    ...
    expect((await api('/me/cycle/predictions?from=2026-13-01&to=2026-01-02')).status).toBe(400)
    expect((await api('/me/cycle/predictions?from=2026-02-02&to=2026-02-01')).status).toBe(400)
    expect((await api('/me/cycle/predictions?from=2020-01-01&to=2026-01-01')).status).toBe(400)
  })
  ...
  test('an unknown time zone is a 400, not a prediction anchored on UTC', async () => {
    const day = todayIn('UTC')
    const res = await api(`/me/cycle/predictions?from=${day}&to=${day}&timeZone=Mars/Olympus`)
  ...
  test('the 400-day cap is inclusive of its own edge', async () => {
    const from = back(200)
    expect(
      (await api(`/me/cycle/predictions?from=${from}&to=${shiftDays(from, 400)}`)).status,
    ).toBe(200)
    expect(
      (await api(`/me/cycle/predictions?from=${from}&to=${shiftDays(from, 401)}`)).status,
    ).toBe(400)
  })
})
...
describe('GET /me/cycle/predictions: what the calendar draws', () => {
  ...
  test('no date outside the range is ever returned', async () => {
    ...
    expect(half.fertileWindow).toEqual(whole.fertileWindow.filter((day) => day <= half.to))
  })
  ...
  test('editing a flow entry moves the prediction on the next read', async () => {
    ...
    const analysis = analyzeCycles(
      {
        days: daysFor(REGULAR_STARTS).filter((day) => day.localDate !== REGULAR_STARTS.at(-1)),
        today: todayIn('UTC'),
        profile: null,
      },
      CYCLE_RULES,
...
describe('GET /me/cycle/predictions refuses rather than failing', () => {
  ...
  test('and still answers 400 for a bad range on that same server', async () => {
    const server = await bootApi(NO_CYCLE_ENV)
    try {
      const res = await apiAt(server.base, '/me/cycle/predictions?from=2026-02-02&to=2026-02-01')
      expect(res.status).toBe(400)
      expect((await json<ErrorBody>(res)).error.code).toBe('VALIDATION')
    } finally {
      server.child.kill()
