 ...
 *
 * The route path carries no payload, address or token (GUARDRAILS 12): it is the one fact
 * that says which request hung, and it holds none of those.
 *
...
export const withRequestTimeout = (
  ...
  return async (request) => {
    ...
    const timer = setTimeout(() => {
      console.error(JSON.stringify({ event: 'request_timeout', method, route }))
    }, timeoutMs)
    try {
